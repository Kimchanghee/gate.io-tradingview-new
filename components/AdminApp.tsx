...
const toDirectionKey = (value?: string): string => {
  if (!value) return '';
  return value.trim().toLowerCase().replace(/[^a-z]+/g, '_');
};

const formatDirectionLabel = ({ direction, action, side }: Pick<AdminSignal, 'direction' | 'action' | 'side'>): string => {
  const directKey = toDirectionKey(direction);
  if (directKey && DIRECTION_LABELS[directKey]) {
    return DIRECTION_LABELS[directKey];
  }

  const actionKey = toDirectionKey(action);
  const sideKey = toDirectionKey(side);

  if (sideKey && actionKey) {
    const canonical = `${sideKey}_${actionKey === 'close' ? 'exit' : actionKey === 'open' ? 'entry' : actionKey}`;
    if (DIRECTION_LABELS[canonical]) {
      return DIRECTION_LABELS[canonical];
    }

    const sideLabel = sideKey === 'long' ? '롱' : sideKey === 'short' ? '숏' : side ?? sideKey;
    let actionLabel = action ?? actionKey;
    if (actionKey === 'open') actionLabel = '진입';
    else if (actionKey === 'close') actionLabel = '청산';
    else if (actionKey === 'ignored') actionLabel = '무시';
    else if (actionKey === 'blocked') actionLabel = '차단';
    return `${sideLabel} ${actionLabel}`.trim();
  }

  if (sideKey) {
    if (DIRECTION_LABELS[sideKey]) {
      return DIRECTION_LABELS[sideKey];
    }
    if (sideKey === 'long') return '롱';
    if (sideKey === 'short') return '숏';
    return side ?? sideKey;
  }

  if (actionKey) {
    if (actionKey === 'open') return '진입';
    if (actionKey === 'close') return '청산';
    if (actionKey === 'ignored') return '무시';
    if (actionKey === 'blocked') return '차단';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <Card title="트레이딩뷰 웹훅 JSON 예시" className="space-y-3">
          ...
          <div className="text-xs text-gray-300 space-y-1">
            <p><strong className="text-gate-primary">indicator</strong>: 웹훅과 연결할 전략/지표 이름입니다. {{strategy.name}} 변수를 그대로 사용하세요.</p>
            <p><strong className="text-gate-primary">symbol</strong>: 알림이 트리거된 티커 혹은 거래쌍입니다. {{ticker}} 변수를 전달하면 거래소 심볼과 일치합니다.</p>
            <p><strong className="text-gate-primary">direction</strong>: 아래 4가지 값 중 하나를 넣어 진입·청산 방향을 표시합니다.</p>
          </div>
...
{`{
  ...
  "size": "{{strategy.order.contracts}}"
          ...
          <div className="text-xs text-gray-400 space-y-1">
            <p>가능한 <code className="bg-gray-900 px-1 py-0.5 rounded border border-gray-700">direction</code> 값:</p>
            <ul className="list-disc list-inside space-y-1">
              <li><code className="bg-gray-900 px-1 py-0.5 rounded border border-gray-700">long_entry</code> → 롱 진입</li>
              <li><code className="bg-gray-900 px-1 py-0.5 rounded border border-gray-700">long_exit</code> → 롱 청산</li>
              <li><code className="bg-gray-900 px-1 py-0.5 rounded border border-gray-700">short_entry</code> → 숏 진입</li>
              <li><code className="bg-gray-900 px-1 py-0.5 rounded border border-gray-700">short_exit</code> → 숏 청산</li>
            </ul>
            <p className="text-[11px] text-gray-500">
              direction 필드를 사용할 수 없는 경우 <code className="bg-gray-900 px-1 py-0.5 rounded border border-gray-700">action</code> (open/close)
              과 <code className="bg-gray-900 px-1 py-0.5 rounded border border-gray-700">side</code> (long/short)을 함께 보내도 동일하게 인식합니다.
            </p>
          </div>
        </Card>
      </div>
              ...
              signals.map((signal) => (
                <div key={signal.id} className="bg-black/40 border border-gray-700 rounded px-3 py-2">
                  ...
                  <div className="text-xs text-gray-500 flex flex-wrap gap-x-3 gap-y-1">
                    <span>지표: {signal.indicator || '-'}</span>
                    <span>방향: {formatDirectionLabel(signal)}</span>
                    <span>심볼: {signal.symbol || '-'}</span>
                    <span>수량: {signal.size ?? '-'}</span>
                    <span>레버리지: {signal.leverage ?? '-'}</span>
                  </div>
                </div>
              ))
            )}
