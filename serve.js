...
const parseDirection = (value) => {
  ...
  const normalised = raw.replace(/[^a-z]+/g, '_');
  const tokens = normalised.split('_').filter(Boolean);

  let side = '';
  let action = '';

  for (const token of tokens) {
    if (!side && (token === 'long' || token === 'short')) {
      side = token;
      continue;
    }
    if (!action && (token === 'open' || token === 'entry' || token === 'enter')) {
      action = 'open';
      continue;
    }
    if (!action && (token === 'close' || token === 'exit' || token === 'flatten')) {
      action = 'close';
      continue;
    }
  }
  ...
  if (!side) {
    if (normalised.includes('long')) side = 'long';
    else if (normalised.includes('short')) side = 'short';
    else if (normalised.includes('sell')) side = 'short';
    else if (normalised.includes('buy')) side = 'long';
  }

  if (!action) {
    if (normalised.includes('close') || normalised.includes('exit') || normalised.includes('flatten')) {
      action = 'close';
    } else if (normalised.includes('sell') && side === 'long') {
      action = 'close';
    } else if (normalised.includes('buy') && side === 'short') {
      action = 'close';
    } else {
      action = 'open';
    }
